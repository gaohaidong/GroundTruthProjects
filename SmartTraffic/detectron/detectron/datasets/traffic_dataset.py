...
class TrafficDataset(object):
    ...
    def image_path_from_index(self,index):
        image_path = os.path.join(_DATASETS[self.name][_IM_DIR], index)
    ...
    def get_roidb(
        ...
        image_ids = self.image_infos.keys()
    ...
    def load_objs_from_index(self, entry):
        import cv2
        index = entry['index']
        im = cv2.imread(os.path.join(_DATASETS[self.name][_IM_DIR], index))
        im_boxes = self.image_infos[index]
        height, width, _ = im.shape
        entry['width'] = width
        entry['height'] = height

        valid_objs = []
        for obj in im_boxes.split(';'):
            if obj == '':
                continue
            cls_name = 'car'
            tmp_obj = dict()
            tmp_obj['cls_name'] = cls_name
            x, y, w, h = map(float, obj.split('_'))
            tmp_obj['bbox'] = [x, y, x + w, y + h]
